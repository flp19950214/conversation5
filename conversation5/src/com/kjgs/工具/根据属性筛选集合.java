...
public class 根据属性筛选集合 {
     ...
     * @return
     */
    public static Object 单个属性筛选集合不为空的第一个对象(JSONArray jsonArray, String arg1){
        JSONArray 单个属性筛选集合不为空的对象 = 单个属性筛选集合不为空的(jsonArray, arg1);
        if(CollectionUtils.isNotEmpty(单个属性筛选集合不为空的对象)){
            return 单个属性筛选集合不为空的对象.get(0);
        }
        return null;
    }
