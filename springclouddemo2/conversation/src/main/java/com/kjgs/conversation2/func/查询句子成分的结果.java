...
@Service
public class 查询句子成分的结果 extends FuncAbstract {
    ...
    public void 执行查询句子成分方法(){
        //获取需要的茶树
        Document 指定下标前面的逻辑成分_无迭代 = Tool.指定下标前面的逻辑成分_无迭代(下标, "执行查询句子成分方法");
        if(!指定下标前面的逻辑成分_无迭代.containsKey(Cons.指向)){
            return;
        }
        Document 执行查询句子成分方法的值 = Tool.指定下标前面的逻辑成分(下标, "执行查询句子成分方法");
