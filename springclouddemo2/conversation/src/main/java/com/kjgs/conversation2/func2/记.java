...
public class 记 extends FuncAbstract {
    ...
    public void init(){
        //加载初始化逻辑语句
        impl逻辑.保存逻辑("《如果》《遇到》《记》《，》《并且》《后面》《1》《个字》《是》《录》，《那就》《把》《当前词》《和》《后面》《1》《个字》《合并为》《1》《个词》");
        impl逻辑.保存逻辑("《如果》《遇到》《记》《，》《并且》《后面》《6》《个字》《是》《录成处理逻辑》《，》《那就》《把》《当前词》《和》《后面》《6》《个字》《合并为》《1》《个词》");
        impl逻辑.保存逻辑("《如果》《遇到》《记》《，》《并且》《后面》《6》《个字》《是》《录当前词对象》《，》《那就》《把》《当前词》《和》《后面》《6》《个字》《合并为》《1》《个词》");
    }
    ...
    public void 分词_记录当前词对象(){
        Document 指定下标的逻辑成分 = Tool.指定下标的逻辑成分(结束下标);
        if (指定下标的逻辑成分 == null) {
            return;
        }
        int 结束下标 =  下标+7;
        String 记录当前词对象 = 逻辑句子.substring(下标, 结束下标);
        if (!"记录当前词对象".equals(记录当前词对象)) {
            return;
        }
        //满足条件 合并果，创建新成分，删除旧成分
        逻辑成分.put(Cons.词语, 记录当前词对象);
        逻辑成分.put(Cons.结束下标, 结束下标);
        Tool.删除指定范围下标的逻辑成分(下标+1, 结束下标);
    }
