...
('获取输入的句子的唯一标识', '查询的对象的键《是》输入的句子；查询的对象的键《是》￥{当前处理的词语}；查询的属性《是》_id；《执行查询对象指定属性值方法》');
('给输入的句子赋值唯一标识', '查询的对象的键《是》输入的句子；查询的对象的键《是》￥{当前处理的词语}；更新的属性《是》_id；《生成对象唯一标识》《作为》更新的属性值；《执行更新对象属性方法》');
('如果输入的句子的唯一标识为空，那么就创建一个', '《获取输入的句子的唯一标识》《作为》被判断的条件；《执行判断是不为空方法》《作为》是否执行判断结果；《给输入的句子赋值唯一标识》');

-- 处理相加功能完整流程
('给当前句子成分添加上级唯一标识', '查询的对象的键《是》对象；查询的对象的键《是》￥{当前处理的词语}；更新的对象类型《是》句子成分；更新的属性《是》上级对象唯一标识；《获取输入的句子的唯一标识》《作为》更新的属性值；《执行更新对象属性方法》');
