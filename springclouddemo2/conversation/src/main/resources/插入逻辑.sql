...
-- 处理相加功能完整流程
('给当前句子成分添加上级唯一标识', '更新的对象《是》当前处理的词语；更新的对象类型《是》句子成分；更新的属性《是》上级对象唯一标识；《获取输入的句子的唯一标识》《作为》更新的属性值；《执行更新对象属性方法》');
('把当前词作为句子的一个成分', '《添加当前词语为句子成分》；《给当前句子成分添加上级唯一标识》');
('获取当前句子成分的数量', '查询的对象《是》输入的句子；查询的属性《是》成分的数量；《执行查询对象指定属性值方法》');
('获取当前句子成分的数量', '');
